...
function SearchDialog({ open, onOpenChange }: SearchDialogProps) {
  ...
  return (
    <div
      className="fixed inset-0 z-50 flex items-start justify-center bg-black/40 px-4 py-24 backdrop-blur-sm"
      role="dialog"
      aria-modal="true"
      aria-label="Search the site"
    >
      <div className="relative w-full max-w-2xl border border-black/10 bg-white shadow-editorial">
        <button
          type="button"
          onClick={() => onOpenChange(false)}
          className="absolute right-4 top-4 text-newspaper-gray transition hover:text-newspaper-ink"
        ...
        <form onSubmit={handleSubmit} className="px-6 pb-6 pt-10 sm:px-10">
          <label
            htmlFor="site-search"
            className="text-xs font-semibold uppercase tracking-[0.35em] text-newspaper-gray"
          >
            Search the archive
          </label>
          <div className="mt-4 flex items-center gap-3 border border-black/10 bg-newspaper-paper px-4 py-3 focus-within:border-newspaper-ink">
            <Search
              size={18}
              strokeWidth={1.5}
              className="text-newspaper-gray"
            />
            <input
              ref={inputRef}
              id="site-search"
            ...
            <button
              ...
              className={cn(
                ...
                query ? "text-newspaper-accent" : "text-newspaper-gray",
              )}
            >
              Enter
            </button>
          </div>
          <p className="mt-4 text-[11px] uppercase tracking-[0.25em] text-newspaper-gray">
